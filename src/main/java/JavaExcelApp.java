...
public class JavaExcelApp {
    ...
    public void writePingdomToExcel(List<PingdomPages> pages, String workBook, String sheet) {
        int row = 0;
    ...
    public void writeGoogleToExcel(List<GooglePages> googlePages, String workBook, String sheet) {
        int row = 0;
        createWorkBook(workBook);
    ...
    /*public static void main(String[] args) throws IOException {
        ...
        Sheet sheet1 = wb.createSheet("DavidPingdom1");
        Row row11 = sheet1.createRow(4);
        Cell cell11 = row11.createCell(2);
        cell11.setCellValue("1,2 sec");
        //Row row12 = sheet1.createRow(4);
        Cell cell12 = row11.createCell(3);
        cell12.setCellValue("1,9 sec");

        Cell cell13 = row11.createCell(4);
        cell13.setCellFormula("LEFT(D5,3)&\" (\"&LEFT(D5,3)-LEFT(C5,3)&\")\"");

        Sheet sheet2 = wb.createSheet("DavidGoogle2");
        Row row2 = sheet2.createRow(4);
        Cell cell2 = row2.createCell(3);
        cell2.setCellFormula("DavidPingdom1!D5 + DavidPingdom1!C5");
//-----------------------------------------------------------тут читаем с файла-----------------
        FileOutputStream fos = new FileOutputStream("WebSiteAudit.xls");
        wb.write(fos);
        fos.close();

        FileInputStream fis = new FileInputStream("WebSiteAudit.xls");
        Workbook wb1 = new HSSFWorkbook(fis);
        String result = wb1.getSheet("DavidPingdom1").getRow(4).getCell(3).getStringCellValue();
